...
import { version, IS_DEV_TEAM } from "@utils/constants";
...
export default {
  ...
  data() {
    return {
      ...
      paths,
      logo:
        "https://s3.ap-northeast-2.amazonaws.com/images.orderhae.com/logo/torder_color_white.png",
      version,
      ...
      try {
        if (!isDev && isLogined) {
          ...
          const formatInitTabletVersion =
            redirectionUrl[redirectionUrl?.length - 1] !== "/"
              ? `${redirectionUrl}/`
              : redirectionUrl;
  ...
  methods: {
              ...
              if (vanData?.responseCode === "0000") {
                ...
                if (!requestCreditItem?.key) {
                  return this.showAlert(
                    `key 값이 없습니다. 에러메세지: ${requestCreditItem?.key}`
                  );
                }
                if (!requestCreditItem?.id) {
                  return this.showAlert(
                    `id 값이 없습니다. 에러메세지: ${requestCreditItem?.id}`
                  );
                }
                if (!requestCreditItem?.creditStat) {
                  return this.showAlert(
                    `stat 값이 없습니다. 에러메세지: ${requestCreditItem?.creditStat}`
                  );
                }
                if (!requestCreditItem?.creditType) {
                  return this.showAlert(
                    `type 값이 없습니다. 에러메세지: ${requestCreditItem?.creditType}`
                  );
                }
                if (!requestCreditItem?.storeCode) {
                  return this.showAlert(
                    `storeCode 값이 없습니다. 에러메세지: ${requestCreditItem?.storeCode}`
                  );
                }
                if (!requestCreditItem?.tabletnumber) {
                  return this.showAlert(
                    `tabletnumber 값이 없습니다. 에러메세지: ${requestCreditItem?.tabletnumber}`
                  );
                }
                if (!requestCreditItem?.tableName) {
                  return this.showAlert(
                    `tableName 값이 없습니다. 에러메세지: ${requestCreditItem?.tableName}`
                  );
                }
                if (!requestCreditItem?.orderkey) {
                  return this.showAlert(
                    `orderkey 값이 없습니다. 에러메세지: ${requestCreditItem?.orderkey}`
                  );
                }
                ...
                return this.showAlert(
                  `잘못된 responseCode 0000입니다. 에러메세지: ${vanData?.errorMessage}`
                );
              }
            } else {
              return this.showAlert(
                `잘못된 callBackPayment message 형태입니다. 출력값: ${msg?.result}`
              );
            }
          }
        } catch (error) {
    ...
    catchOffline() {
      window.addEventListener("offline", () => {
        ...
        const payload = {
          visible: true,
          message:
            "인터넷이 연결 되지 않았습니다. 인터넷 연결 확인 후 새로고침 해주세요."
        };
        ...
        if (this.$store.state.auth?.store?.store_code) {
          ...
          if (!isDev && isLogined) {
            const redirectionUrl =
              res.data.data.T_order_store_orderView_version;
    ...
    updatePhoneNumber(key) {
      if (/^\d+$/g.test(key)) {
        ...
        if (this.phoneNumber.length < 13) {
          this.phoneNumber = this.autoHypenPhone(
            this.phoneNumber + String(key)
          );
        }
      } else if (key == "d") {
    ...
    async applyHappyTalk() {
      try {
        ...
        if (res.data?.code !== 200) {
          const errorMessage =
            res.data?.message || "상담 신청에 실패하였습니다.";
    ...
    onTouchSecretFunction() {
      ...
      this.secretFunctionTouchTimer = setTimeout(
        function() {
          this.resetSecretFunctionTouchCount();
        }.bind(this),
        1000
